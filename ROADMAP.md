...
- [x] Cross-check every factual claim destined for public Envoy collateral (Charter, README, website copy) for accuracy and supporting citation (sweep `02-plans/claim-verification/01-sweep.md`; surgical edits applied)
- [x] Remove any comparative claims about other projects from public-facing documents; retain only neutral references where required for interoperability context (e.g. `SKILL.md` format)
...
- Voice channel (ASR (e.g. Whisper) → Envoy → TTS)
...
- Cross-runtime conformance vectors at every release gate per the runtime-abstraction spec partition (byte-identical on spec paths; semantically-equivalent on LLM-composed paths; per `specs/runtime-abstraction.md` §5)
