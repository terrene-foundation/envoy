...
5. `enabled_at` within 365 days (annual re-attestation; clock injected for determinism).
...
Both signatures are Ed25519 over `template_envelope_hash` — the canonical artifact both parties attest to deploying. Signature verification routes through the shared `envoy/registry/steward_quorum.py::verify_steward_quorum` primitive (1-of-1 quorum per key) — no parallel verifier.
