...
**Session-store local-file-read residual (Phase 02 — vault-sibling session DB) — CLOSED (S5o-enc).** The vault-sibling session store (`<vault-stem>.session.db`, the `pending_grant` + `session_observed_state` tables in `specs/session-runtime.md`) now holds its `state_json` snapshots and `request_json` / `resolution_json` rows AES-256-GCM-encrypted-at-rest, not canonical-JSON plaintext. The encryption key is keychain-gated — a dedicated 32-byte AES-256 key (`SESSION_ENCRYPTION_KEY_ID`) persisted in the OS keychain alongside, but namespaced distinctly from, the Ed25519 session signing key. The defenses now stack: 0o600 permissions on the whole SQLite family (main + `-wal` / `-shm`); the OS-keychain Ed25519 `resolution_sig` tamper anchor over each resolution row (a row mutated outside the keystore-backed write path fails verification on read); AND confidentiality-at-rest of the payload columns. A local-file read — by a malicious same-user process or an unencrypted-disk forensic read — recovers only ciphertext; without OS-keychain access the payloads cannot be decrypted, and a byte-level ciphertext tamper fails the AES-256-GCM tag check on read. Index/key columns (`request_id` / `session_id` / `principal_id` / `state` / `version` / timestamps / `ttl_expires_at`) stay cleartext so lookups, the `CHECK` constraint, and the lost-update `version` re-check keep working. **Design note:** the closure is keychain-gated rather than vault-passphrase-gated (the durable-substrate todo originally anticipated "vault-unlock key material") — the `SessionRouter` opens short-lived one-shot CLI processes (`grant approve` in a fresh process) that hold no passphrase, so a passphrase-gated key would force a passphrase prompt on every `grant`/`chat` invocation; keychain-gating closes the same local-file-read threat without that UX cost, matching the existing `resolution_sig` signing key's trust model. Remaining (narrower) residual: an attacker who can both read the file AND use the OS keychain as the logged-in user (i.e. is already executing as that user with keychain access) can still decrypt — this is the device-compromise class, outside the local-file-read threat this closure targets.
