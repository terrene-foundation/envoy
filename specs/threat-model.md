...
**Session-store local-file-read residual (Phase 02 — vault-sibling session DB).** The vault-sibling session store (`<vault-stem>.session.db`, the `pending_grant` + `session_observed_state` tables in `specs/session-runtime.md`) holds `state_json` snapshots and `request_json` / `resolution_json` rows as canonical JSON, not ciphertext. Current mitigation: 0o600 permissions on the whole SQLite family (main + `-wal` / `-shm`) so a non-owner local user cannot read it, and an OS-keychain Ed25519 `resolution_sig` over each resolution row (a tamper anchor: a row mutated outside the keystore-backed write path fails verification on read). Documented residual: the snapshot/request/resolution payloads are NOT encrypted-at-rest, so an attacker who reads the file as the owning user (e.g. a malicious process running as that user, or an unencrypted-disk forensic read) recovers plaintext session-observation state. Encryption-at-rest for these payload columns depends on the vault-unlock key lifecycle (tracked in the Phase-02 durable-substrate workspace todo), so the present posture is 0o600 + signing.
