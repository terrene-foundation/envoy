...
**Status: ✅ velocity half COMPLETE** (2026-06-15, PR #112). **Delegation half → S4g-2b ✅ COMPLETE (2026-06-15, with F12-b — see below).**
...
## S4g-2b — 3-deep delegation-tree literal cascade e2e (✅ COMPLETE — landed with F12-b)

**Status: ✅ COMPLETE** (2026-06-15, with **F12-b**). The EC-8 cascade e2e (`tests/e2e/test_grant_moment_3_resolution_shapes_with_cascade.py::TestEC8CascadeRevocationRealInfra`) now drives a REAL `TrustStoreAdapter` delegation tree (root + 3 descendants) through the production binding chain `revoke_prior_grant` → `CascadeRevocationOrchestrator.revoke_and_verify` → `KailashPyRuntime.trust_cascade_revoke` → F12-b bridge → async `TrustStoreAdapter.revoke`, replacing the stubbed `cascade_responses`. A second real-infra test asserts the verify-or-raise contract (incomplete tree → `CascadeIncompleteError` names the missing children). Closes the `grant-moment.md` spec gap (split-state "Phase 02 lifts the test" framing removed per `spec-accuracy.md` Rule 2).

**Scope correction (empirical):** the spec (`specs/grant-moment.md` § test list) mandates **"root + 3 expected descendants"** — a **breadth-3** tree (one delegator, three children), satisfiable with a SINGLE per-principal adapter (F12-a's pattern extended to 3 children). The earlier "multi-adapter Genesis orchestration (root→child→grandchild→ggc)" framing was an over-statement of the requirement; no cross-principal chain is needed for the spec's acceptance shape.

**F12-b — sync↔async cascade facade bridge: ✅ COMPLETE** (2026-06-15, PR pending). `envoy/runtime/adapters/_async_cascade_bridge.py::run_coro_blocking` drives the async revoke on a dedicated worker-thread event loop (loop-safe inside or outside a running loop — the EC-8 hazard a naive `asyncio.run()` cannot handle), with a bounded wait (`AsyncBridgeTimeoutError` — a wedged cascade fails loud, never a silent retained-authority hang). Both `KailashPyRuntime` AND `KailashRsBindingsRuntime` route the async-store case through the SAME shared helper (per `autonomous-execution.md` Rule 4 + `security.md` dual-halves discipline). Gates: reviewer + security-reviewer (no CRIT/HIGH; in-shard fixes for the bridge timeout, observability, e2e connection cleanup, conformance-comment + rs-guard-message stragglers).

**F12-c — production revoke entrypoint (CLI) — OPEN, blocked on a cross-thread store precondition.** **Value-anchor (per `value-prioritization.md` MUST-2):** wires `revoke_prior_grant` to a user-facing `envoy grant revoke` command so a real operator can cascade-revoke a grant (the EC-8(c) charter constraint, `01-analysis/02-mvp-objectives.md` EC-8). **Precondition (security-reviewer F12-b MED-3):** the bridge runs `revoke` on a worker thread; a production CLI that `await store.initialize()`s on the main loop's thread and then drives the sync facade with that PRE-initialized adapter would reuse a thread-affine `sqlite3` connection cross-thread → `ProgrammingError` (fail-loud) or, on a permissive config, a silent no-op write (retained authority). F12-c MUST either (a) drive a FRESH / lazily-initialised adapter through the bridge (the tested-safe pattern), OR (b) make the kailash chain/posture sub-stores open connections per-operation on the executing thread (the `store.py:944-956` bc/digest precedent already does this — the chain/posture sub-stores are the inconsistent ones). Until F12-c lands, cascade-revoke is reachable through `revoke_prior_grant` (tested) but has no production CLI callsite.
