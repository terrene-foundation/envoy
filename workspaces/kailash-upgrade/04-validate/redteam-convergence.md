...
| 3   | 23 `ResourceWarning: unclosed database`                  | 2.29.3 `SQLitePostureStore.__init__` opens a persistent SQLite conn eagerly; envoy's adapter built it in `__init__` → leak on construct-without-initialize | lazy-init `_posture_store` (construct in `initialize()`, close+reset in `close()`, typed guard per `zero-tolerance.md` Rule 3a); restores "no I/O until initialize()" |
