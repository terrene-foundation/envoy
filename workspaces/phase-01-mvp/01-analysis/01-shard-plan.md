...
**Wave-F completion (2026-05-04):** Shards 20, 21, 22 — DONE (with mid-wave account-limit recovery: 3 follow-up agents finished 2 plans + 7 flows + 2 additive specs that were lost when initial agents hit limit).

| Shard | Status | Headline                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| ----- | ------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 20    | DONE   | 4 plans: 01-build-sequence (topo-sorted primitive build order), 02-test-strategy (3-tier per `rules/testing.md`), 03-package-skeleton (`envoy-agent` PyPI + `envoy/` Python package layout, no `src/` indirection, NOTICES with LGPL-3.0+ python-telegram-bot disclosure), 04-redteam-cycle-plan (9 mechanical sweeps + adversarial trigger for shards 4/5/17 if round 1 returns 0/0; counter resets on any new finding or feature).                                                                                                                                                           |
| 21    | DONE   | 8 user flows (install + boundary conversation + grant moment + daily digest + channel onboarding + shamir backup + ledger export + posture ratchet). 5 cross-flow invariants surfaced (Trust Vault as cross-flow state surface; cold-storage co-location of Shamir + trust-anchor; plain-language error mapping per-spec-error; fail-closed defaults compose; timezone Option A highest UX visibility in Flow 04). 1 HIGH UX risk on Flow 04 — Singapore user sees digest at 4pm local under Option A; **recommendation: EC-7 N=3 cohort MUST include at least one non-UTC user before ship.** |
| 22    | DONE   | 1 unique HIGH (timezone, consolidated 11+12; Option A vs Option B framed; human decision at shard 25); 1 HIGH-candidate HELD (shard 13 chat-completion); 11 MED (3 closed by additive `specs/independent-verifier.md`, 8 dispositioned as additive prose); 39 LOW (no Phase 01 action). 2 additive new specs drafted: `specs/independent-verifier.md` + `specs/mvp-build-sequence.md`. **Zero edits to existing frozen specs — zero MUST Rule 5b 37-sibling sweeps triggered.** `specs/_index.md` updated with the 2 new manifest rows.                                                        |

Wave F preserved the no-existing-spec-edit discipline. Wave G (shards 23–24 red team rounds + shard 25 closure) follows.
