...
**Criterion 2 — BET falsifiability cost:** BET-8 ("Foundation Health uptake") is the single BET that explicitly hinges on Heartbeat shipping in Phase 01 (per `00-inheritance-from-phase-00.md` § 2.4 BET table — "YES if Heartbeat ships in Phase 01; NO if de-scoped"). BET-3 (sovereignty durability), BET-5 (Ledger as daily artifact), and BET-12 (governance-primary-surface palatability) lose their `[Heartbeat]`-tagged falsifying-evidence bullets per thesis § 5.0 (`[Heartbeat]` tag means "requires Phase 02 Heartbeat availability"). However — and this is the load-bearing observation — the thesis itself **already tags every BET-3 / BET-12 falsifying bullet as `[Heartbeat]`**, which means thesis § 5.0 already presumes Heartbeat is a Phase 02 measurement substrate. **The falsifiability cost of de-scope is BET-8 only.** BET-3 / BET-12 are NOT measurable at Phase 01 cohort scale even with Heartbeat shipping (k=100 floor — see § 3.3 above). **WEIGHT: LOW AGAINST DE-SCOPING.** BET-8 was always the bet that hinged on this decision.
