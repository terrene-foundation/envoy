...
**Methodology:** CARE / EATP / CO / COC (CC BY 4.0). Envoy code: Apache 2.0.
...
An _envoy_ is an agent that carries delegated authority from a sovereign. In Envoy's architecture this is literally true — every action is backed by a cryptographically signed EATP Delegation Record rooted in a named human's Genesis Record. No grant, no action.
...
| **Runtime**         | Pluggable — `kailash-rs-bindings` (Rust-accelerated, free) or `kailash-py` (pure Python, Apache 2.0 — references CC BY 4.0 specs). First-run picker. |
...
2. **Governance as the source of performance.** Pre-compiled envelope checks meet bounded per-class latency budgets (≤5ms structural, ≤50ms semantic-cached; full per-spec budgets in `specs/envelope-model.md` §80). Typed plan DAGs are envelope-verified at the envelope intersection so safe parallelism is structurally available (per `specs/sub-agent-delegation.md`; binding integration tracked in kailash-rs#509 / #510). Declared envelopes mean no mid-stream permission prompts. Rust hot path by default. Governance _is_ the source of speed.
3. **Features, first-class and seamless.** Phase-tagged channel coverage (Phase 01: CLI + Web; Phase 02: 6 messaging; Phase 04: 23+ total), `SKILL.md` compatibility, local-first model option, one-command install, mobile-first onboarding, Shamir recovery.
...
Apache 2.0 for Envoy code. Content and methodology: CC BY 4.0, Terrene Foundation. Runtime bindings: see DECISIONS §ADR-0009 for the full license posture.
