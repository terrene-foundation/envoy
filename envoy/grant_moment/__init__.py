...
T-03-50 ships the foundation: the M0→M4 state machine, the JCS+NFC-canonicalized
...
Subsequent shards layer on the dispatch surfaces (T-03-51:
``OutOfEnvelopeDetector`` + ``ChannelHandoff``), the cross-grant orchestration
(T-03-52: ``CascadeRevocationOrchestrator`` + ``PlanSuspensionBridge``), and
the novelty-aware friction classifier (T-03-53: ``NoveltyClassifier``).
